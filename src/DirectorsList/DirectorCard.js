...
export const DirectorCard = props => {
  const DirectorName = props => {
    ...
    return (
      <Box bg={'yellow.400'} p={0}>
        <HStack>
          <Box textStyle="h2" p={1} color={textColor}>
            {props.director}
          </Box>
          <Spacer />
          <IconButton
      ...
      <Box minH={'80px'} bgGradient={movieBg} p={2} ml={2} mt={2} pb={4}>
        ...
        <Flex spacing={1} overflow="hidden">
          <Box flex={1} fontWeight={'normal'}>
            <Link>
              <Text noOfLines={2}>{props.movieName}</Text>
            </Link>
          </Box>
  ...
  return (
    ...
    >
      ...
      <MovieItem movieName={'Movie Name Movie Name Movie Name'} />
      <MovieItem movieName={'Movie Name 2'} />
    </Box>
  );
};
